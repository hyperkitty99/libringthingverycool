...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		textAnim.setFormat(Paths.font("DIN2014Bold.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function reloadBGs() {
		...
		var bg:FlxSprite = new FlxSprite(-600 + OFFSET_X - playerXDifference, -300).loadGraphic(Paths.image('stageback'));
		bgLayer.add(bg);

		var stageFront:FlxSprite = new FlxSprite(-650 + OFFSET_X - playerXDifference, 500).loadGraphic(Paths.image('stagefront'));
